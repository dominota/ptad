...
CompressiveTracker::CompressiveTracker(void)
{
	...
	rSearchWindow2 = 10; // size of search window //yqh small window
	learnRate = 0.9;//0.85f;	// Learning rate parameter//yqh: the weight to remember the last param 
...
void CompressiveTracker::processFrame(Mat& _frame, Rect& _objectBox,bool & tracked)
{
    ...
    if(radios[radioMaxIndex]>ratiosInit)//radioMaxLast*0.8)
	{
		...
		sampleRect(_frame, _objectBox, rSearchWindow1, rOuterPositive+4.0, 1000,	sampleNegativeBox);
...
/*	float tempBoxX=0, tempBoxY=0;
	...
	if(tempBoxN>0)
	{
		...
		sampleRect(_frame, tempBox, rSearchWindow2,detectBox);
		getFeatureValue(imageIntegral, detectBox, detectFeatureValue);
		DrawPaticleWeights(WeightImg, detectBox, detectFeatureValue.cols, radios);
	}*/
	sampleRect(_frame, detectBox[_radioMaxIndex], rSearchWindow2,detectBox);
	getFeatureValue(imageIntegral, detectBox, detectFeatureValue);
	radioClassifier(detectFeatureValue,_radioMaxIndex);
	DrawPaticleWeights(WeightImg, detectBox, detectFeatureValue.cols, radios);
